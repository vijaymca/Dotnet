...
//Filename      : WellboreAdvSearch.ascx.cs
#endregion
...
{
    /// <summary>
    /// Code behind for Wellbore Advanced Search. This is for Wellbore 
    /// Advanced search types.
    /// </summary>
    public partial class WellboreAdvSearch :UIHelper
    {

        #region Declaration
        const string PAGENAME = "AdvSearchWellWellbore.aspx";
        const string WELLBOREREPORT = "Wellbore";
        const string WELLTYPESTATUS = "WellExistenceKind";
        const string WELLBORECHECKREFRESH = "WellboreCheckRefresh";
        const string SAVESEARCHNAMEQUERYSTRING = "savesearchname";
        const string ASSET = "asset";

        const string WELLBOREASSET = "wellbore";
        #endregion
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string strOnChange = string.Format("javascript:FileSearchTypeSelectedIndexChange(this,'{0}|input','{1}|input','{2}|input','{3}|input')", txtUnique_Wellbore_Identifier.ID, txtWellbore_Common_Name.ID, txtWellbore_Name.ID, txtAlias_Name.ID);

            cboSearchCriteria.Attributes.Add("onchange", strOnChange);
            cmdSaveSearch.Attributes.Add("onclick", "return ValidateWellboreSaveSearch('" + WELLBOREREPORT + "');");
            ...
            if(!Page.IsPostBack)
            {
                try
                {
                    //Sets the Wellbore Check refresh session variable.
                    CommonUtility.SetSessionVariable(Page, WELLBORECHECKREFRESH, DateTime.Now.ToString());
                    if(Request.QueryString[ASSET] != null)
                    {
                        if(Request.QueryString[ASSET].ToLower().Equals(WELLBOREASSET))
                        {

                            rdoRbDate.Items[1].Text = "Completion";
                            rdoRbDate.Items[1].Value = "Completion";
        ...
        protected void cmdReset_Click(object sender, EventArgs e)
        {
            ...
            if(string.Equals(cmdSaveSearch.Text.ToString(), MODIFYSRCH))
            {
                if(Request.QueryString[SAVESEARCHNAMEQUERYSTRING] != null)
                {
                    ...
                    strUrl = PAGENAME + "?asset=Wellbore" + "&manage=true&savesearchname=" + Request.QueryString[SAVESEARCHNAMEQUERYSTRING].ToString() + "&operation=modify";
                }
                else
                {
                    strUrl = PAGENAME + "?asset=" + "Wellbore";
                }
            }
            else
            {
       ...
       * ******************************************************************************/
        ...
        protected void cboSavedSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                /// Changed By: Yasotha
                /// Date : 19-Jan-2010
                xmldoc = ((MOSSServiceManager)objMossController).GetDocLibXMLFile(WELLBOREADVANCEDSEARCH, strUserID);
                ClearUIControls();
                #endregion
        ...
        protected void lstCountry_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                /// Added for WDA Search Enhancements on 31-May-2010
                lblAPIArea_Identifier.Visible = false;
                ...
                /// Added for WDA Search Enhancements on 31-May-2010
                lstAPIAreas_Identifier.Items.Clear();
                lstAPIAreas_Identifier.Enabled = false;
                ...
                /// Added for WDA Search Enhancements on 31-May-2010
                chbAPIAreas_Identifier.Checked = false;
                chbAPIAreas_Identifier.Enabled = false;
                ...
                else
                {
                    ...
                    /// Added for WDA Search Enhancements on 31-May-2010
                    lstAPIAreas_Identifier.Enabled = true;

                    chbState.Enabled = true;
                    chbField_Identifier.Enabled = true;

                    /// Added for WDA Search Enhancements on 31-May-2010
                    chbAPIAreas_Identifier.Enabled = true;
                }
            }
            catch(WebException webEx)
            {
        ...
        /// <summary>
        /// Handles the Change event of the chbAPIAreas_Identifier control.
        /// Populates the API Areas Identifier for the selected coutry.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="System.EventArgs"/> instance containing the event data.</param>
        protected void chbAPIAreas_Identifier_Change(object sender, EventArgs e)
        {
            ArrayList arrListValue = new ArrayList();
            XmlDocument xmlDoc = new XmlDocument();
            ...
            try
            {
                if(chbAPIAreas_Identifier.Checked)
                {
                    arrListValue.Add(lstCountry);
                    ...
                    SetListValues(arrListValue); //parameter for which data needs to be populated.   
                    objRequestInfo.Entity.Name = APIIDENTIFIER;
                    xmlDoc = objReportController.GetSearchResults(objRequestInfo, intListValMaxRecord, APIIDENTIFIER, null, 0);
                    if(xmlDoc != null)
                    {
                        objUIUtilities.AssignFieldValuesToDropDown(xmlDoc, lstAPIAreas_Identifier);
                    }
                    if(lstAPIAreas_Identifier.Items.Count < 1)
                    {
                        lblAPIArea_Identifier.Visible = true;
                        lblAPIArea_Identifier.Text = APIAREANOTFOUNDMESSAGE;
                        lblAPIArea_Identifier.ForeColor = System.Drawing.Color.Red;
                    }
            ...
            catch(SoapException soapEx)
            {
                ...
                {
                    lblAPIArea_Identifier.Text = soapEx.Message;
                }
                lblAPIArea_Identifier.ForeColor = System.Drawing.Color.Red;
            }
            catch(WebException webEx)
        ...
        private void LoadOtherPageControls()
        {
            ...
            if(Request.QueryString[SAVESEARCHNAMEQUERYSTRING] != null)
            {
                ...
                if(Request.QueryString["manage"] != null)
                {
                    ...
                    /// Changed By: Yasotha
                    /// Date : 19-Jan-2010
                    strAdvSearchName = WELLBOREADVANCEDSEARCH;
                    #endregion
        ...
        private void BindTooltipTextToControls()
        {
            ...
            string strAsset = WELLBOREREPORT;
            try
            {
                ...
                if(dtFilterTooltip.Rows.Count > 0)
                {
                    foreach(DataRow drFilter in dtFilterTooltip.Rows)
                    {
                        ...
                        if(string.Equals(imgUWBI.ID.Remove(0, 3).ToString(), strControlName))
                        {
                            imgUWBI.ToolTip = strTooltipText;
                        }
                        ...
                        else if(imgWellboreStatus.ID.Remove(0, 3).ToString().Equals(strControlName))
                        {
                            imgWellboreStatus.ToolTip = strTooltipText;
                        }
                        else if(imgSearchName.ID.Remove(0, 3).ToString().Equals(strControlName))
                        {
                            imgSearchName.ToolTip = strTooltipText;
                        }
                        else if(imgAPIAreaIdentifier.ID.Remove(0, 3).ToString().Equals(strControlName))
                        {
                            imgAPIAreaIdentifier.ToolTip = strTooltipText;
                        }
                        else if(imgBlock.ID.Remove(0, 3).ToString().Equals(strControlName))
                        {
                            imgBlock.ToolTip = strTooltipText;
                        }
                        else if(imgLeaseIdentifier.ID.Remove(0, 3).ToString().Equals(strControlName))
                        {
                            imgLeaseIdentifier.ToolTip = strTooltipText;
                        }
                    }
                }
            }
        ...
        * ******************************************************************************/
        ...
        private ArrayList SetBasicAttributeGroup()
        {
            ...
            /// Changed By: Yasotha
            /// Date : 19-Jan-2010
            objBasicAttributeGroup.Name = WELLBOREREPORT;
            #endregion
        ...
        private ArrayList SetBasicAttribute()
        {
            ...
            {
                arrAttribute = SetSearchCriteriaAttribute(arrAttribute, txtUnique_Wellbore_Identifier, ReadFileSearch(fileUploader.PostedFile, hidWordContent.Value), cboSearchCriteria);
            ...
            if(!string.Equals(cboSearchCriteria.SelectedItem.Text, UWBI))
            {
                arrAttribute = SetBasicListAttribute(arrAttribute, txtUnique_Wellbore_Identifier);
            }
            ...
            /// Added for WDA requirements.
            if(Request.QueryString.Count > 0 && Request.QueryString[ASSET] != null && Request.QueryString[ASSET].ToLowerInvariant().Equals(WELLBOREASSET))
            {
                /// Added for WDA requirements.
                arrAttribute = SetBasicListAttribute(arrAttribute, lstAPIAreas_Identifier, true);
                arrAttribute = SetBasicListAttribute(arrAttribute, txtBlocks);
                arrAttribute = SetBasicListAttribute(arrAttribute, txtLease_Identifier);
            }
            ...
            if(!string.IsNullOrEmpty(txtWellbore_Name.Text.Trim()))
                arrAttribute = SetBasicListAttribute(arrAttribute, txtWellbore_Name);
            if(!string.IsNullOrEmpty(txtWellbore_Common_Name.Text.Trim()))
                arrAttribute = SetBasicListAttribute(arrAttribute, txtWellbore_Common_Name);
        ...
        protected void DisplaySearchResults()
        {
            ...
            string strAssetType = "Wellbore";
            UISaveSearchHandler objUISaveSearchHandler = new UISaveSearchHandler();
            objUISaveSearchHandler.DisplayResults(Page, objReqInfo, WELLBOREADVANCEDSEARCH);
            RedirectPage(SEARCHRESULTSPAGE + "?SearchType=" + WELLBOREADVANCEDSEARCH + "&asset=" + strAssetType, strAssetType);
        }
       ...
       * ******************************************************************************/
        ...
        private void BindCheckBoxControl(string childControlID)
        {
            ...
            switch(childControlID)
            {
                ...
                //====================================
                //Module Name:WDA Search Criteria Enhancements
                //Description: Setting the selected API and Block from Saved Search
                //Date: 8-June-2010
                //Modified Lines: 2090 -2101
                //====================================

                /// Whether the ListBox is same as API Identifier then check the required CheckBoxes.
                case "lstAPIAreas_Identifier":
                    {
                        chbAPIAreas_Identifier.Checked = true;
                        chbAPIAreas_Identifier_Change(null, EventArgs.Empty);
                        break;
                    }
                default:
                    break;
